...
ConfigPtr parseConfig(const std::string& filename) {
    ...
    for (const auto& operation_node : operations_node) {
      ...
      if (config->operations[operation_name]) {
        ...
        config->operations[operation_name] =
            generateOperation(operation_name, config->store, n);
      }
    }
  }
  ...
  if (yaml_config["mqtt"]) {
    ...
    if (mqtt["broker"]) {
      ...
      config->mqtt = std::make_unique<MqttConnection>(
          config->store, broker, realm, username, password);
    }
  }
...
Config::Config()
    : fps(30), width(100), store(std::make_shared<VariableStore>()) {}
