...
struct Timing {
  ...
  void report() {
    for (auto& e : measurements) {
      ...
      std::cerr << name << " average: " << m.average << " max: " << m.max
                << " min: " << m.min << " count: " << m.count << std::endl;
      m.reset();
    }
  }
};
...
class Measure {
 ...
 public:
  Measure(const std::string& name, Timing& t)
      : timing(t), measurement(t.measurements[name]) {}
...
void WorkerThread::run() {
  while (doRun) {
    ...
    Operation::BufferType buffer(
        std::make_unique<AllocatedBuffer<HSV>>(config->width));
