...
enum action_type { SUBSCRIBE, UNSUBSCRIBE, MESSAGE, MQTT_MESSAGE, QUIT };

typedef typename std::variant<std::vector<uint8_t>,
...
class broadcast_server {
 public:
  ...
  void run(uint16_t port) {
    ...
    const auto log_levels = websocketpp::log::alevel::connect |
  ...
  bool has_clients() const { return !m_connections.empty(); }
  ...
  void process_messages() {
    while (1) {
      ...
      if (a.type == SUBSCRIBE) {
        ...
        for (con_list::iterator it = m_connections.begin();
        ...
        for (it = m_connections.begin(); it != m_connections.end(); ++it) {
          m_server.send(*it, static_cast<void*>(bytes.data()), bytes.size(),
