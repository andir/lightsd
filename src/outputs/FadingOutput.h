...
class FadingOutput {
 ...
 public:
  FadingOutput(FrameScheduler& scheduler, ChildT& child)
      : scheduler(scheduler), child(child) {}

  void draw(const std::vector<HSV>& buffer) { _draw(buffer); }

  void draw(const AbstractBaseBuffer<HSV>& buffer) { _draw(buffer); }
 ...
 private:
  ...
  void _draw(const Container& buffer) {
    ...
    std::transform(lastFrame.begin(), lastFrame.end(), buffer.begin(),
                   diff.begin(), [steps](const auto& lastf, const auto& newf) {
                     //                           auto diff = HSV{
                     //                                   (newf.hue - lastf.hue)
                     //                                   / steps,
                     //                                   (newf.saturation -
                     ...
                     // std::cout << "( " << newf.string() << " - " <<
                     // lastf.string() << ") / " << steps << " = " <<
                     // diff.string();
                     // std::cout << diff.string() << "\t" << newf.string() << "
                     // " << lastf.string() << "\n";
                     return delta;
    ...
    for (size_t i = 0; i < steps; i++) {
      ...
      size_t counter = 0;
      std::transform(
          lastFrame.begin(), lastFrame.end(), diff.begin(), lastFrame.begin(),
          [this, &buffer, &i, &counter](auto& lastf, const auto& difff) {
            const auto f = lastf + difff;
            //                std::cout << "(" << i << ")" << lastf.string() <<
            //                " + " << difff.string() << " = " << f.string() <<
            //                std::endl;
            //                std::cout << "target: " <<
            //                buffer.at(counter).string() << std::endl;
            //                counter ++;
            ...
            //                if (counter > 0) {
            //                    volatile const auto target =
            //                    buffer.at(counter);
            //                    volatile const auto ptarget =
            //                    buffer.at(counter - 1);
            //
