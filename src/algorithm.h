...
inline constexpr ColorType LinearColor(const float step) {
  static_assert(always_fail<ColorType>::value,
                "No LinearColor implemented for this type yet.");
  return ColorType{};
}
...
struct Mask {
  ColorSystem operator()(const float factor, const ColorSystem& color) const {
    static_assert(always_fail<ColorSystem>::value,
                  "Please implement masking for your type");
  }
};
...
void MaskBuffer(const std::vector<float>& mask, BufferType& data) {
  ...
  std::transform(mask.begin(), mask.end(), data.begin(), data.begin(),
                 Mask<ValueType>());
}
...
void PartialMaskBuffer(const std::vector<float>& mask,
                       Container& data,
                       const size_t offset) {
  if (mask.size() == 0 || data.size() == 0 || mask.size() > data.size())
    return;
  ...
  std::transform(mask_begin, mask_end, data_begin, data_begin,
                 Mask<value_type>());
}
...
inline T2& convert(
    typename std::enable_if<std::is_same<T1, T2>::value, T2>::type& a) {
  return a;
};
