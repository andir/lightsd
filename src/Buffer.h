...
class Iterator {
 ...
 public:
  Iterator(const BufferType* const buffer, size_t pos, bool end = false)
      : buffer(buffer), pos(pos), end(end) {}

  Iterator(this_type&& other) : buffer(other.buffer), pos(other.pos) {}

  Iterator(const this_type& other) : buffer(other.buffer), pos(other.pos) {}
  ...
  bool operator==(const this_type& other) const { return pos == other.pos; }
  ...
  inline value_type& operator*() const { return buffer->at(pos); }
...
class AbstractBaseBuffer {
 public:
  ...
  IteratorType end() const { return IteratorType(this, count(), true); }

  ReverseIteratorType rend() const { return ReverseIteratorType(this, 0); }
...
class AllocatedBuffer : public AbstractBaseBuffer<ValueType> {
 ...
 public:
  ...
  AllocatedBuffer(const size_t count)
      : ptr(std::make_unique<value_array_type>(count)), _count(count) {
    // allocate memory and in case we've to exit this function early, release
    // it.
    assert(ptr != nullptr || count == 0);

    std::cerr << "Constructed AllocatedBuffer. data begin: "
              << (void*)&(*ptr.get()) << std::endl;
  }
  ...
  AllocatedBuffer(AllocatedBuffer&& other)
      : ptr(other.ptr.release()), _count(other._count) {
    std::cerr << "Copy-constructed AllocatedBuffer. data begin: "
              << &(*ptr.get()) << std::endl;
  }

  AllocatedBuffer& operator=(AllocatedBuffer&& rhs) = default;

  virtual ~AllocatedBuffer() { std::cerr << "Destructing buffer" << std::endl; }
  ...
  inline IteratorType begin() const { return IteratorType(this, 0); }

  inline IteratorType end() const { return IteratorType(this, count(), true); }
...
class BufferView : public AbstractBaseBuffer<typename ValueType::value_type> {
 public:
  ...
  using UpperT =
      std::shared_ptr<AbstractBaseBuffer<typename ValueType::value_type>>;
 ...
 public:
  ...
  BufferView(UpperT& upper, size_t offset, size_t len)
      : upper(upper), offset(offset), _count(len) {}
  ...
  inline IteratorType end() const { return IteratorType(this, _count, true); }
};
