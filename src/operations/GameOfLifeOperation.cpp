...
GameOfLifeOperation::GameOfLifeOperation(const std::string& name,
    ...
    : Operation(name, store, start, end),
      initialized(false),
      frame_counter(0),
      default_hue(name + "/default_hue",
                  Operation::HSV_HUE,
                  store,
                  getValueByKey<float>("default_hue", start, end, 180.0f)),
      default_saturation(
          name + "/default_saturation",
      ...
      v0(name + "/v0",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v0", start, end, false)),
      v1(name + "/v1",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v1", start, end, true)),
      v2(name + "/v2",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v2", start, end, true)),
      v3(name + "/v3",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v3", start, end, false)),
      v4(name + "/v4",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v4", start, end, true)),
      v5(name + "/v5",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v5", start, end, true)),
      v6(name + "/v6",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v6", start, end, true)),
      v7(name + "/v7",
         Operation::BOOLEAN,
         store,
         getValueByKey<bool>("v7", start, end, false)),
      speed(name + "/speed",
            Operation::BOOLEAN,
            store,
            getValueByKey<float>("speed", start, end, 1.0f)),
      randomizeColor(name + "/randomizeColor",
                     Operation::BOOLEAN,
                     store,
...
void GameOfLifeOperation::update(const size_t width, const size_t fps) {
  ...
  std::transform(delta.begin(), delta.end(), output.begin(), output.begin(),
                 [](const HSV& a, const HSV& b) -> HSV {
                   return HSV{a.hue + b.hue, a.saturation + b.saturation,
                              a.value + b.value};
                 });
...
Operation::BufferType GameOfLifeOperation::operator()(
