...
class SequenceOperation : public Operation {
        ...
        void loadSequence(const std::string& name, std::shared_ptr<VariableStore>& s, YAML::const_iterator start, YAML::const_iterator end);
public:
        SequenceOperation(const std::string& name, std::shared_ptr<VariableStore>& s, YAML::const_iterator& start, YAML::const_iterator& end);
