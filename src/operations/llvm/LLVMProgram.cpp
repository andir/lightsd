...
template <typename DataType = float,
          size_t stack_size = 32 / sizeof(DataType),
          size_t n_op_arg = (sizeof(DataType) / sizeof(uint8_t))>
class LLVMProgram {
 public:
  enum OpCode {
    BRK,
 ...
 private:
  ...
  using const_data_it =
      typename std::array<DataType, stack_size>::const_iterator;
 ...
 public:
  LLVMProgram(const std::vector<uint8_t>& ucode) {
    ...
    std::transform(ucode.begin(), ucode.end(), code.begin(),
                   [](uint8_t v) { return OpCode(v); });
  }
  ...
  DataType run(std::vector<DataType> arguments) const {
    ...
    for (const_code_it code_it = code.cbegin(); code_it != code.cend();
         ++code_it) {
      ...
      switch (uint8_t(op)) {
        ...
        case OpCode::ADD:
          do_binary(stack_it, [](DataType a, DataType b) { return a + b; });
          break;
        case OpCode::MUL:
          do_binary(stack_it, [](DataType a, DataType b) { return a * b; });
          break;
        ...
        case OpCode::DIV:
          ...
          break;
        case OpCode::SIN:
          do_unary(stack_it, [](DataType a) { return std::sin(a); });
          break;
        case OpCode::COS:
          do_unary(stack_it, [](DataType a) { return std::cos(a); });
          break;
        case OpCode::MIN:
          do_binary(stack_it,
