...
class FadeOperation : public Operation {
  ...
  inline auto getCallback() {
    return [this](ValueType*) { this->recalcMask(); };
  }

 public:
  FadeOperation(const std::string& name,
      ...
      : Operation(name, store, start, end),
        min(name + "/min",
            Operation::FLOAT_0_1,
            store,
            getValueByKey<float>("min", start, end),
            getCallback()),
        max(name + "/max",
            Operation::FLOAT_0_1,
            store,
            getValueByKey<float>("max", start, end),
            getCallback()),
        from(name + "/from",
             Operation::INT,
             store,
             getValueByKey<int>("from", start, end),
             getCallback()),
        to(name + "/to",
           Operation::INT,
           store,
           getValueByKey<int>("to", start, end),
           getCallback()) {
    recalcMask();
  }
