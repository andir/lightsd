...
LuaOperation::LuaOperation(const std::string& name,
                           std::shared_ptr<VariableStore>& store,
                           YAML::const_iterator begin,
                           YAML::const_iterator end)
    : Operation(name, store, begin, end), luaState(openLua()) {
  ...
  if (!mqtt) {
    ...
    if (fn == "") {
      std::cerr << "No lua file configure for plugin" << this->getName()
                << std::endl;
    } else {
      luaL_dofile(luaState, fn.c_str());
    }
  } else {
    mqtt_code = std::make_unique<BoundConcreteValue<std::string>>(
        name + "/code", Operation::STRING, store,
        getValueByKey<std::string>("code", begin, end, ""),
        [this](ValueType* vt) { this->reloadCode(vt->getString()); });
